...
export default function InvestmentPortfolio() {
  ...
  const handleRefresh = () => {
    ...
    setInvestments((prev) =>
      prev.map((inv) => ({
        ...inv,
        value: inv.value + (Math.random() - 0.5) * 200,
        subInvestments: inv.subInvestments.map(subInv => ({
          ...subInv,
          value: subInv.value + (Math.random() - 0.5) * 100,
        }))
      }))
    );
  };
  ...
  return (
    <div className="bg-gradient-to-br from-emerald-50 to-teal-100 min-h-screen p-4 pb-20">
      {/* Header */}
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-bold text-emerald-800">Invest</h1>
        ...
        <Button
          ...
          className="rounded-full hover:bg-emerald-200 transition-colors border-emerald-300"
        >
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-emerald-200 p-2 flex justify-around items-center rounded-t-3xl shadow-lg">
        <a className="flex flex-col items-center p-2 rounded-full text-gray-600" href="/dashboard">
          <Home className="w-6 h-6" />
          <span className="text-xs mt-1">Home</span>
        </a>
        <a className="flex flex-col items-center p-2 rounded-full text-emerald-600" href="/dashboard/invest">
          <TrendingUp className="w-6 h-6" />
          <span className="text-xs mt-1">Invest</span>
        </a>
        <a className="flex flex-col items-center p-2 rounded-full text-gray-600" href="/dashboard/learn">
          <BookOpen className="w-6 h-6" />
          <span className="text-xs mt-1">Learn</span>
        </a>
        <a className="flex flex-col items-center p-2 rounded-full text-gray-600" href="/dashboard/settings">
